...
include("utils/xmlrpcs_emu.inc");

/*************************************************************
* This is the standard server example from the useful inc    *
* xmlrpc php distribution. The only modifications are the    *
* above two includes have been changed to use the xmlrpc-epi *
* emulation classes.                                         *
*************************************************************/
...
function findstate($m) {
  ...
  if ($err) {
		$resp = new xmlrpcresp(0, $xmlrpcerruser, $err);
      return $resp;
  } else {
		// otherwise, we create the right response
		// with the state name

		$resp = new xmlrpcresp(new xmlrpcval($sname));
      return $resp;
  }
}
...
function i_whichToolkit($m) {
	...
	return new xmlrpcresp ( php_to_val($ret));
}
